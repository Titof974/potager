...
async fn main() {
    ...
    let v2 = Router::new()
        ...
        .route(
            ...
            put(upload_blob_with_reference),
        )
        .route(
            "/v2/:name/blobs/uploads/:reference",
            patch(patch_upload_blob),
        )
        .route("/v2/:name/manifests/:reference", get(with_status))
        .route("/v2/:name/manifests/:reference", head(manifest_exists))
        .route("/v2/:name/manifests/:reference", put(with_status))
...
pub async fn patch_upload_blob_with_reference(
    ...
    Response::builder()
        .header(
            "Location",
            format!(
                "http://172.26.16.1:3000/v2/{}/blobs/uploads/{}",
                name, my_uuid
            ),
        )
        .status(StatusCode::ACCEPTED)
...
async fn upload_blob(
    ...
    if headers.contains_key("content-length") && headers.get("content-length").unwrap().eq("0"){
        ...
        .header("Location", format!(
            "http://172.26.16.1:3000/v2/{}/blobs/uploads/{}",
            name, request_chunck_upload.session_id
        ))
        .status(StatusCode::ACCEPTED)
        .body(Body::from(""))
        .unwrap()
